...
def process_mathematical_parser(file, password_list):
    ...
    # ENGINE 2: HEAVY-DUTY STREAM EXTRACTION (Data Padhna)
    try:
        ...
        txn_kws = [
            "RTGS", "NEFT", "UPI", "IMPS", "CHQ", "ATM", 
            "WITHDRAW", "DR", "DEBIT", "CHARGES", "FEE", "INTEREST"
        ]

        # SELF-HEALING MATH ENGINE
        for i in range(len(raw_transactions)):
            ...
            if i > 0:
                ...
                if any(kw in curr["Narration"].upper() for kw in txn_kws): 
                    curr["Debit"] = curr["Amount"]
                else: 
                    curr["Credit"] = curr["Amount"]
